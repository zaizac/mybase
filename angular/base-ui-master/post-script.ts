...
(async () => {
  ...
  await fs.copy(sources, destination, (err: any) => {
    if (err) {
      console.log('ui-master assets > An error occured while copying the folder.');
      return console.error(err);
    }
    console.log('ui-master assets > Copy completed!');
    ...
    fs.copy(fontSources, fontDestination, (err: any) => {
      ...
      console.log('ui-master assets > Copy completed for font-awesome fonts!');
...
async function copyFilesIntoExistingFolder(fontSourceFolder: string, fontDestFolder: string, logName: string) {
  ...
  for (const file of files) {
    await fs.copy(fontSourceFolder + '/' + file, fontDestFolder + '/' + file, (err: any) => {
      if (err) {
        console.log('ui-master assets > An error occured while copying the files.');
        return console.error(err);
      }
    });
  }
  const outputName = logName ? logName : 'files';
  console.log('ui-master assets > Copy completed for ' + outputName);
}
...
async function bundleScss() {
  ...
  if (imports) {
    ...
    if (filesNotFound.length) {
      console.error(`ui-master assets > SCSS imports failed \n\n${filesNotFound.join('\n - ')}\n`);
  ...
  if (found) {
    ...
    console.log('ui-master assets > SCSS bundle success');
    // copy themify-icons
    copyFilesIntoExistingFolder(
