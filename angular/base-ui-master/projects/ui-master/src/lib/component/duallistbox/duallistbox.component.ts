...
import { DualListModel } from './dualListModel';

export type compareFunction = (a: any, b: any) => number;
let nextId = 0;

@Component({
  selector: 'ui-duallistbox',
...
export class DuallistboxComponent implements DoCheck, OnChanges {
  ...
  static APPLY_LIST_NAME = 'applyed'
  ...
  available: DualListModel;
  confirmed: DualListModel;
  applyed: DualListModel;

  sourceDiffer: any;
  destinationDiffer: any;

  applyValue: Array<any>;

  private sorter = (a: any, b: any) => (a._name < b._name) ? -1 : ((a._name > b._name) ? 1 : 0);

  constructor(private differs: IterableDiffers) {
    this.available = new DualListModel(DuallistboxComponent.AVAILABLE_LIST_NAME);
    this.confirmed = new DualListModel(DuallistboxComponent.CONFIRMED_LIST_NAME);
    this.applyed = new DualListModel(DuallistboxComponent.APPLY_LIST_NAME);
  }
  ...
  ngOnChanges(changeRecord: { [key: string]: SimpleChange }) {
    ...
    if (changeRecord['format']) {
      ...
      if (typeof (this.format.apply) === 'undefined') {
        this.format.apply = DuallistboxComponent.DEFAULT_FORMAT.apply;
      }
  ...
  dragEnd(list: DualListModel = null): boolean {
    if (list) {
  ...
  drag(event: DragEvent, item: any, list: DualListModel) {
  ...
  allowDrop(event: DragEvent, list: DualListModel): boolean {
  ...
  drop(event: DragEvent, list: DualListModel) {
    if (event.dataTransfer.types.length && (event.dataTransfer.types[0] === this.id)) {
      event.preventDefault();
      this.dragLeave();
      this.dragEnd();
  ...
  private makeUnavailable(source: DualListModel, item: any) {
  ...
  moveItem(source: DualListModel, target: DualListModel, item: any = null, trueup = true) {
    let i = 0;
  ...
  shiftClick(event: MouseEvent, index: number, source: DualListModel, item: any) {
  ...
  selectApplyItem(item: any) {
    return Object.values(item);
  }

  selectAll(source: DualListModel) {
    source.pick.length = 0;
    source.pick = source.sift.slice(0);
  }

  selectNone(source: DualListModel) {
    source.pick.length = 0;
  }

  isAllSelected(source: DualListModel): boolean {
  ...
  isApplyEnable(source: DualListModel): boolean {
    if (source.list.length === 0 || source.list.length === source.pick.length) {
      this.applyValue = source.list;
      return true;
    }
    return false;
  }

  isAnySelected(source: DualListModel): boolean {
  ...
  private unpick(source: DualListModel) {
  ...
  clearFilter(source: DualListModel) {
    if (source) {
      source.picker = '';
      this.onFilter(source);
    }
  ...
  applyvalue() {
    console.log('selected value', this.applyValue);
  }
}
