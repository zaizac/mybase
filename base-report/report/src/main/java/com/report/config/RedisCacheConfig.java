...
import com.report.config.cache.CustomRedisCacheManager;
import com.report.config.cache.JsonRedisSerializer;
...
public class RedisCacheConfig extends CachingConfigurerSupport {
	...
	public CacheManager cacheManager(RedisTemplate<String, String> redisTemplate) {
		...
		cacheManager.setDefaultExpiration(RptCacheConstants.CACHE_DURATION_DAILY);
	...
	private void clearCacheBucket(CacheManager cacheManager) {
		...
		if (!BaseUtil.isObjNull(cache)) {
			LOGGER.info("CLEARING CACHE BUCKET ~ {}", RptCacheConstants.CACHE_BUCKET);
			cache.clear();
		}
	}
