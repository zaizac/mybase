...
public class FcmService extends GenericAbstract {
	...
	@Autowired
	NotFcmResponseService notFcmResponseSvc;
	 ...
	 */
	public String sendNotiFcm(final String subject, final String emailTO, final String emailCC,
			final Map<String, Object> emailParam, final String templateCode, Integer notifyId) {

		NotEmailTemplate temp = notEmailTemplateSvc.findByTemplateCode(templateCode);

		String notifyContent = temp.getEmailContent();
		String token = emailCC;
		if(BaseUtil.isObjNull(token)
				&& !BaseUtil.isObjNull(emailTO)) {
			// Retrieve active fcmToken from
			FcmDevice fcmDevice = new FcmDevice();
			fcmDevice.setStatus(true);
			Fcm fcm = new Fcm();
			fcm.setUserId(emailTO);
			List<FcmDevice> fcmDevices = getIdmService().searchFcmDevice(fcmDevice);
			if(!BaseUtil.isListNullZero(fcmDevices)) {
				List<String> strFcmTokens = new ArrayList<>();
				fcmDevices.stream().forEach(t -> {
					if(!BaseUtil.isObjNull(t.getFcmToken())) {
						strFcmTokens.add(t.getFcmToken());
					}
				});
				token = strFcmTokens.toArray(new String[strFcmTokens.size()]).toString();
			}
		}
		
		notifyContent = notifyContent.replace("token", token);
		notifyContent = notifyContent.replace("subject", subject);
		

		try {
			ObjectNode tempJson = JsonUtil.objectMapper().readValue(notifyContent, ObjectNode.class);
		...
		try {
			callRestEndpoint(HttpMethod.POST, notifyContent, notifyId);
		} catch (Exception e) {
			LOGGER.error("Exception: sendNotiFcm: ", e);
		}
	 ...
	 */
	public String callRestEndpoint(HttpMethod httpMethod, String jsonStr, Integer notifyId) throws IOException {
		...
		try {
			// Save response to DB
			ObjectNode respObj = JsonUtil.objectMapper().readValue(result, ObjectNode.class);	
			NotFcmResponse resp = new NotFcmResponse();
			resp.setNotifyId(notifyId);
			resp.setContent(jsonStr);
			resp.setMulticastId(respObj.get("multicast_id").toString());
			resp.setSuccess(respObj.get("success").asInt());
			resp.setFailure(respObj.get("failure").asInt());
			resp.setCanonicalId(respObj.get("canonical_ids").asInt());
			resp.setResults(respObj.get("results").toString());
			notFcmResponseSvc.create(resp);
		} catch (IOException e) {
			LOGGER.error("IOException: ", e);
		}
