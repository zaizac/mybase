...
import com.idm.config.cache.CustomRedisCacheManager;
import com.idm.config.cache.JsonRedisSerializer;
...
public class RedisCacheConfig extends CachingConfigurerSupport {
	...
	@Value("${" + BaseConfigConstants.REDIS_CONF_DURATION + "}")
	private Integer redisDuration;

	@Value("${" + BaseConfigConstants.REDIS_CONF_BUCKET + "}")
	...
	public JedisConnectionFactory redisConnectionFactory() {
		...
		sb.append("Password : " + redisPword + BaseConstants.NEW_LINE);
		sb.append("Duration : " + redisDuration + BaseConstants.NEW_LINE);
		sb.append("Bucket Name : " + redisBucket);
		sb.append(BaseConstants.NEW_LINE + BaseConstants.LOG_SEPARATOR + BaseConstants.NEW_LINE);
		LOGGER.info("{}", sb);
	...
	public CacheManager cacheManager(RedisTemplate<String, String> redisTemplate) {
		...
		cacheManager.setDefaultExpiration(redisDuration);
	...
	private void clearCacheBucket(CacheManager cacheManager) {
		Cache cache = cacheManager.getCache(redisBucket);
		if (!BaseUtil.isObjNull(cache)) {
			LOGGER.info("CLEARING CACHE BUCKET ~ {} ", redisBucket);
			cache.clear();
		}
	}
