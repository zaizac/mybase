...
import com.be.config.cache.CustomRedisCacheManager;
import com.be.config.cache.JsonRedisSerializer;
...
public class RedisCacheConfig extends CachingConfigurerSupport {
	...
	public CacheManager cacheManager(RedisTemplate<String, String> redisTemplate) {
		...
		cacheManager.setDefaultExpiration(BeCacheConstants.CACHE_DURATION_DAILY);
	...
	private void clearCacheBucket(CacheManager cacheManager) {
		Cache cache = cacheManager.getCache(BeCacheConstants.CACHE_BUCKET);
		if (!BaseUtil.isObjNull(cache)) {
			LOGGER.info("CLEARING CACHE BUCKET ~ {} ", BeCacheConstants.CACHE_BUCKET);
			cache.clear();
		}
	}
