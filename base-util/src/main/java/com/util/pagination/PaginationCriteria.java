...
public class PaginationCriteria {
	...
	public Integer getPageSize() {
		return (null == pageSize) ? 0 : pageSize;
	}
	 ...
	 * @return the order by clause
	 */
	public String getOrderByClause(String table) {
		...
		if (!isSortByEmpty()) {
			...
			while (sbit.hasNext()) {
				...
				if (null == sbsb) {
					sbsb = new StringBuilder();
					sbsb.append(ORDER_BY).append(table + "." + pair.getKey()).append(SPACE)
							.append(pair.getValue());
				} else {
					sbsb.append(COMMA).append(table + "." + pair.getKey()).append(SPACE).append(pair.getValue());
				}
			}
		}
