//
//  RegisterEmployerInfoViewController.swift
//  MyCare
//
...
class RegisterEmployerInfoViewController: UIViewController, Alert {
    ...
    @IBOutlet private weak var companyContactTF: UITextField!
    ...
    override func viewWillAppear(_ animated:Bool) {
        ...
        //notificationCenter.addObserver(self, selector: #selector(keyboardWillShow(aNotification:)), name: UIResponder.keyboardWillShowNotification, object: nil)
    ...
    private func initialConfig() {
        print("Saved Reg Address Info: ", AppData.registerAddressInfo)

        title = Constants.empInfo.localized()
        ...
        self.companyContactTF.delegate = self
        ...
        self.companyContactTF.keyboardType = .numberPad
        
//        self.addCompulsoryMark(forView: self.companyNameTF)
//        self.addCompulsoryMark(forView: self.companyContactTF)
        //self.companyNameTF.addAsterisk()
        //self.companyContactTF.addAsterisk()
    ...
    private func setText() {
        ...
        self.companyNameTF.placeholder = Constants.companyName.localized()
        self.companyContactTF.placeholder = Constants.contactNo.localized()
        
        self.postalCodeTF.placeholder = Constants.postalCode.localized()
        self.mukimTF.placeholder = Constants.selectMukim.localized()
        self.districtTF.placeholder = Constants.district.localized()
        self.stateTF.placeholder = Constants.state.localized()
    ...
    private func nextAction(_ sender: UIButton) {
        ...
        */
        ...
        if !validated.isSuccess {
            ...
            // Save values in userdefaults - dict type
            
            let address = self.addressTV.text ?? ""
            
            
                        
            // there is no param for emp Districts Code
            ...
            AppData.registerEmployerInfo = dictEmployerInfo as Dictionary<String, Any>
    ...
    private func checkForValidation() -> (message: String, isSuccess: Bool) {
        ...
        guard let companyContact = self.companyContactTF.text, companyContact != "" else {
            return (Constants.enterCompanyContactNumber.localized(), false)
        }
        
        guard companyContact.isPhoneNumber else {
            return (Constants.companyContactNumberNotCorrect.localized(), false)
        }
        
        if companyContact.count < 10 {
...
extension RegisterEmployerInfoViewController: UITextFieldDelegate {
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        ...
        switch textField {
        case companyNameTF:
            companyContactTF.becomeFirstResponder()
        default:
            textField.resignFirstResponder()
        }
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        // 2
        if textField == companyContactTF  && textField.text?.count == 11 {
            return false
        } else if textField == postalCodeTF && textField.text?.count == 5 {
            return false
        }
...
extension RegisterEmployerInfoViewController: UITextViewDelegate {
...
extension RegisterEmployerInfoViewController: UISearchResultsUpdating {
...
// MARK: - API methods
extension RegisterEmployerInfoViewController {
        ...
        service.listOfStatesService(requestObj: stateRequest, Success: { [weak self] (responses, serverObj) in
            DispatchQueue.main.async {
                if responses.count > 0 {
                    ...
                    self?.arrAllStatesDesc.sort(by: { (value1, value2) -> Bool in
        ...
        service.listOfDistrictService(requestObj: districtRequest, Success: { [weak self] (responses, serverObj) in
            DispatchQueue.main.async {
                if responses.count > 0 {
                    ...
                    self?.arrAllDistrictsDesc.sort(by: { (value1, value2) -> Bool in
                        return value1 < value2
                    })
                }
        ...
        service.listOfCityService(requestObj: townRequest, Success: { [weak self] (responses, serverObj) in
            DispatchQueue.main.async {
                if responses.count > 0 {
                    ...
                    self?.arrAllTownsDesc.sort(by: { (value1, value2) -> Bool in
                        return value1 < value2
                    })
        ...
        service.listOfMukimService(requestObj: townRequest, Success: { [weak self] (responses, serverObj) in
            DispatchQueue.main.async {
                if responses.count > 0 {
                    ...
                    self?.arrAllMukimsDesc.sort(by: { (value1, value2) -> Bool in
                        return value1 < value2
                    })
                }
...
extension RegisterEmployerInfoViewController : CustomPickerDelegate {
    // Delegates
    ...
    func addCustomPicker() {
        ...
        // disable keyboard
        self.postalCodeTF.resignFirstResponder()
        self.view.endEditing(true)
    }
...
extension RegisterEmployerInfoViewController : LocationUpdateProtocol {
    // MARK: ~ Core Location
    // Negative cases
...
extension RegisterEmployerInfoViewController: GMSAutocompleteResultsViewControllerDelegate {
    func resultsController(_ resultsController: GMSAutocompleteResultsViewController,
        ...
        // Place name. Ex: Bestinet Sdn Bhd
        if let placeName = place.name, placeName != "" {
            self.companyNameTF.text = placeName.uppercased()
        }
        
        // Address components
        self.addressTV.text = place.formattedAddress?.uppercased()
        for component in place.addressComponents! {
            print("component.types: ", component.types)
