...
import com.report.sdk.model.ErrorResponse;
...
public class ReportRestTemplate {
	...
	private static final String URL_EMPTY_MESSAGE = "'url' must not be null";
	...
	public <T> T getForObject(String url, Class<T> responseType, Map<String, Object> urlVariables) {
		Asserts.notEmpty(url, URL_EMPTY_MESSAGE);
	...
	public byte [] postForByteArray(String url, Object requestBody, 
			Map<String, Object> uriVariables) {
		Asserts.notEmpty(url, URL_EMPTY_MESSAGE);
		url = UriUtil.expandUriComponent(url, uriVariables);
		LOGGER.info("RPT POST Request: {}", url);
	...
	public <T> T putForObject(String url, Object requestBody, Class<T> responseType,
		...
		try {
			...
			if (httpResponse.getStatusLine().getStatusCode() != 200) {
				mapHttpResponse(httpResponse, url);
			}
	...
	private void mapHttpResponse(CloseableHttpResponse httpResponse, String url) {
		try {
			String result = EntityUtils.toString(httpResponse.getEntity());
			ErrorResponse er = new ObjectMapper().readValue(result, ErrorResponse.class);
			throw new ReportException(er.getErrorCode(), er.getErrorMessage());
