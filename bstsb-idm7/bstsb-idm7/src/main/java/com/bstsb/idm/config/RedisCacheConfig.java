...
import com.bstsb.idm.config.cache.CustomRedisCacheManager;
import com.bstsb.idm.config.cache.JsonRedisSerializer;
...
public class RedisCacheConfig extends CachingConfigurerSupport {
	...
	public CacheManager cacheManager(RedisTemplate<String, String> redisTemplate) {
		...
		cacheManager.setDefaultExpiration(IdmCacheConstants.CACHE_DURATION_DAILY);
	...
	private void clearCacheBucket(CacheManager cacheManager) {
		Cache cache = cacheManager.getCache(IdmCacheConstants.CACHE_BUCKET);
		if (!BaseUtil.isObjNull(cache)) {
			LOGGER.info("CLEARING CACHE BUCKET ~ {} ", IdmCacheConstants.CACHE_BUCKET);
			cache.clear();
		}
	}
