...
public class UidGenerator implements InitializingBean {
	...
	private static final String INT_CHAR = "123456789";
	...
	public String getGeneratedPwd() {
		...
		for (int i = 0; i < uidLength; i++) {
			int spot = random.nextInt(INT_CHAR.length());
			uid.append(INT_CHAR.charAt(spot));
		}
		return uid.toString();
	}
	...
	public String getTransactionId() {
		...
		if (StringUtils.hasText(val) && org.apache.commons.lang3.StringUtils.isNumeric(val)) {
			if (org.apache.commons.lang3.StringUtils.length(val) > getTxnIdLength()) {
				txnCode = org.apache.commons.lang3.StringUtils.substring(val, 0, getTxnIdLength());
			} else {
				txnCode = val;
			}
		}
