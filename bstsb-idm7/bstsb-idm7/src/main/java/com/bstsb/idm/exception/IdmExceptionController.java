 ...
 */
@ControllerAdvice
public class IdmExceptionController {

	private static final Logger LOGGER = LoggerFactory.getLogger(IdmExceptionController.class);
	...
	public @ResponseBody ErrorResponse handleError404(HttpServletRequest request, Exception e) {
		IdmException idmEx = new IdmException(IdmErrorCodeEnum.E404IDM001);
		return processError(idmEx, "404");
	}
	...
	private ErrorResponse processError(Exception ex, String errorCode) {
		LOGGER.error("IDM Response Error: {} - {}", errorCode, ex.getMessage());
